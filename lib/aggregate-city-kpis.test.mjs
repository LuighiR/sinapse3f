...
test("merges hourly rows by hour and sets peakHour", () => {
  ...
  a.calls.summary = {
    ...
    received: { count: 7 },
    lost: { count: 1 },
    lostWithoutEmployee: { count: 2 },
    totalInbound: { count: 8 },
  }
  a.calls.unansweredAnswered = { count: 4 }
  ...
  b.calls.summary = {
    ...
    received: { count: 5 },
    lost: { count: 3 },
    lostWithoutEmployee: { count: 1 },
    totalInbound: { count: 8 },
  }
  b.calls.unansweredAnswered = { count: 5 }

  const g = aggregateCityKpis([a, b], "2026-07-01", "2026-07-02")
  assert.equal(g.calls.summary.received.count, 12)
  assert.equal(g.calls.summary.lost.count, 4)
  assert.equal(g.calls.summary.lostWithoutEmployee.count, 3)
  assert.equal(g.calls.unansweredAnswered.count, 9)
...
test("sums money without IEEE float artifacts", () => {
  const a = emptyCityKpi("2026-07-01", "2026-07-02")
  a.budgets.total = { count: 1, value: "10.1" }
  a.sales.total = { count: 2, value: "10.1" }
  const b = emptyCityKpi("2026-07-01", "2026-07-02")
  b.budgets.total = { count: 1, value: "20.2" }
  b.sales.total = { count: 1, value: "20.2" }

  const g = aggregateCityKpis([a, b], "2026-07-01", "2026-07-02")
  assert.equal(g.budgets.total.value, "30.3")
  assert.equal(g.sales.total.value, "30.3")
  assert.equal(g.sales.averageTicket.value, "10.1")
  // 2 inclusive days → 30.3 / 2
  assert.equal(g.sales.averageDaily.value, "15.15")
})
