...
test("planDiretoriaBranches only links employee.branchId", () => {
...
test("fetchGerenciaKpis employee mode fetches only employee.branchId with erpId", async () => {
  ...
  assert.ok(kpiUrls.length > 0, "expected KPI requests for employee branch")

  for (const u of kpiUrls) {
    assert.doesNotMatch(u, /branchId=2/, "other branches must not be requested")
    assert.doesNotMatch(u, /branchId=3/, "other branches must not be requested")
    assert.match(u, /branchId=1/)
  }

  const commerce = kpiUrls.filter(
    (u) =>
      u.includes("/kpis/budgets/") ||
      u.includes("/kpis/sales/"),
  )
  const calls = kpiUrls.filter((u) => u.includes("/kpis/calls/"))
  const wa = kpiUrls.filter(
    (u) =>
      u.includes("/kpis/whatsapp/") &&
      !u.includes("/tags/hourly/comparison"),
  )

  assert.ok(commerce.length > 0)
  ...
  for (const u of wa) {
    assert.doesNotMatch(u, /sellerId=/)
    assert.match(u, /chatId=chat-1/)
  }

  assert.equal(result.branches.length, 3)
  assert.deepEqual(result.failedBranchIds, [])
  assert.equal(result.branches.find((b) => b.branch.id === 2)?.data.budgets.total.count, 0)
...
test("fetchGerenciaKpis keeps budgets when calls/whatsapp fail for employee branch", async () => {
  const from = "2026-07-01"
  const to = "2026-07-31"
  ...
  assert.equal(result.branches[0].data.calls.summary.totalInbound.count, 0)
  assert.equal(result.branches[0].data.whatsapp.summary.totalConversations.count, 0)
})
