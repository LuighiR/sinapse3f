...
function stubKpiJson(url, from, to) {
  ...
  if (url.includes("/kpis/calls/drilldown")) {
    return {
      period,
      filters: {},
...
test("fetchGerenciaKpis employee mode sends internal branch.id as branchId", async () => {
  ...
  assert.ok(
    kpiUrls.some(
      (u) =>
        u.includes("/kpis/calls/drilldown") &&
        u.includes("status=answered") &&
        u.includes("outcome=UNANSWERED"),
    ),
    "expected unanswered answered drilldown count",
  )
