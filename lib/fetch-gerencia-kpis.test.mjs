...
test("fetchGerenciaKpis employee mode sends sellerId per linked branch and skips unlinked", async () => {
  ...
  // Commerce KPIs must carry sellerId; calls must not; whatsapp summary must not.
  const commerce1 = branch1.filter(
    (u) =>
      u.includes("/kpis/budgets/") ||
      u.includes("/kpis/sales/") ||
      u.includes("/kpis/budgets/follow-up"),
  )
  const calls1 = branch1.filter((u) => u.includes("/kpis/calls/"))
  const wa1 = branch1.filter(
    (u) =>
      u.includes("/kpis/whatsapp/") &&
      !u.includes("/tags/hourly/comparison") &&
      !u.includes("/tags/comparison"),
  )
  assert.ok(commerce1.length > 0)
  for (const u of commerce1) {
    assert.match(u, /sellerId=101/)
    assert.doesNotMatch(u, /extensionUuid=/)
    assert.doesNotMatch(u, /chatId=/)
  }
  for (const u of calls1) {
  ...
  for (const u of branch2.filter(
    (x) => x.includes("/kpis/budgets/") || x.includes("/kpis/sales/"),
  )) {
...
test("fetchGerenciaKpis keeps budgets when calls/whatsapp fail for a linked branch", async () => {
  const from = "2026-07-01"
  const to = "2026-07-31"
  ...
  const result = await fetchGerenciaKpis({
    ...
    employee: {
      ...
      erpUsers: [{ id: 10, erpId: 42754, branchId: 2 }],
      erpId: 42754,
    },
  })
